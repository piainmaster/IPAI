...
def knncalc(k, feature_list):
    knn_list = list()
    feature_list.sort(key=lambda tup: sum(tup[1]))  # nach den Werten in der zweiten Spalte sortieren (kng)
    feature_list.sort(key=lambda tup: math.sqrt(sum(x ** 2 for x in tup[1])))
    for i in range(k):
        knn_list.append(feature_list[i][0])  # es werden "synthetic" und "genuine" eingetragen
...
validation_data = []
validation_data = combine_list_with_genuine(data_PLUS_spoofed)
...
validation_features = [cv2.resize(img, (736, 192)) for img in
...
for i, (train_index, test_index) in enumerate(loo.split(validation_images)):
    ...
    # global variance
    test_variance = validation_labels[test_index]
    variance1_distances = []
    test_variance10 = calculate_variances(validation_images[test_index], 10)
    ...
    """
    # loop over all genuine and 003 data
    for j in range(len(images)):
        varrrrr = calculate_variances(validation_images[test_index], 1)
        variance1_distances.append(
            [variance_list1[j][0], variance_list1[j][1][0] - calculate_variances(validation_images[test_index], 1)])
        variance10_distances.append([variance_list10[j][0],
                                     variance_list10[j][1][0] - calculate_variances(validation_images[test_index],
                                                                                    10)])  # todo. abs macht differenz zwischen einzelnen Werten aber nicht eine allgemeine Distanzmessung -> knn wirft error
        """
        variance20_distances.append([variance_list20[j][0], abs(variance_list20[j][1][0] - calculate_variances(validation_images[test_index], 20))])
        variance30_distances.append([variance_list30[j][0], abs(variance_list30[j][1][0] - calculate_variances(validation_images[test_index], 30))])
        variance40_distances.append([variance_list40[j][0], abs(variance_list40[j][1][0] - calculate_variances(validation_images[test_index], 40))])
        variance50_distances.append([variance_list50[j][0], abs(variance_list50[j][1][0] - calculate_variances(validation_images[test_index], 50))])
        variance100_distances.append([variance_list100[j][0], abs(variance_list100[j][1][0] - calculate_variances(validation_images[test_index], 100))])
        """
    # for j in train_index:
    ...
    # prediction for current test image
    if knncalc(k_knn, variance1_distances) == test_variance: correct_variance1_preds += 1
    if knncalc(k_knn, variance10_distances) == test_variance: correct_variance10_preds += 1
    """
